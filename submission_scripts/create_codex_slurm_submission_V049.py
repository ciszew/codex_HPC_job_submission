...
================================================================================
Version: 49 (Security-Hardened, Cascading Config, Robust Orchestration)
...
Changes from V48:
  - All shell interpolations use shlex.quote() or double-quoted bash variables.
  - Generated Python literals use repr() / json.dumps() to prevent injection.
  - Silent 'except OSError: pass' replaced with logged errors that halt execution.
  - Raw sbatch calls replaced with submit_chain() — validates numeric Job IDs,
    executes scancel rollback on failure.
  - Hardcoded paths replaced with 4-priority cascading config resolution
    (CLI > Env Vars > Config File > Script Defaults).
  - Case-insensitive CSV header mapping with BOM stripping.
  - Wall-time estimates clamped to MAX_WALL_HOURS (168h / 7 days).
"""
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("--- MCMICRO SUBMISSION SUMMARY (V49) ---")
    print("="*60)
