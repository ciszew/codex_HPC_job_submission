...
================================================================================
Version: 52 (Optimized Resource Estimator & Dynamic Quantification)

Changes from V051:
  - BUG FIX: params-quant.yml 'tma' parameter is now dynamic (based on IS_TMA_WORKFLOW).
             In V051, 'tma: true' was hardcoded, causing quantification failures on
             Whole Slide Image (WSI) datasets since it looked for dearrayed cores.
  - OPTIMIZATION: Overhauled estimate_resources() to use sub-linear scaling functions
             (power-law time scaling and square-root memory scaling) instead of linear/hardcoded values.
             Reduces whole-cell segmentation memory from 160GB to ~59GB for 80GB WSI data,
             allowing WSI jobs to run in the tier1q partition (<=128GB) with much faster queue times.
             Increases average CPU/memory efficiency across both small and large datasets.
  - IMPROVEMENT: format_slurm_time() now uses math.ceil() to ensure safe rounding for time limits.
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("--- MCMICRO SUBMISSION SUMMARY (V52) ---")
    print("="*60)
...
for item in registration background dearray segmentation quantification_merged qc_overlays_wc qc_overlays_nuc qc_report_wc.html qc_report_nuc.html QA_QC_Report.txt; do
    ...
    rsync -ah --info=progress2 "{scratch_run_dir}/$item" "{source_data_dir}/" 2>/dev/null
    RSYNC_ITEM_EXIT=$?
